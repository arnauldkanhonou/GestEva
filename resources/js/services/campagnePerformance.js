...
export default function useCampagnePerformance() {
    const errors = ref('');
    const tabError = ref('');
    let campagne = reactive({id: '', annee: '',typeevaluation: '', libelle: '', plage1: '', plage2: '', cloturer: ''});
    const campagnes = ref([]);
    const annees = ref([]);
    ...
    const getCampagne = async (id) => {
        axios.get('campagneperformances/' + id)
            .then((response) => {
                if (response.data.data.code === 500) {
                    ...
                    campagne.typeevaluation = response.data.data.type_evaluation_id;
    ...
    const getCampagnes = async () => {
        axios.get('campagneperformances')
            .then((response) => {
                if (response.data.data.code === 500) {
                    Swal.fire({
                        title: 'Erreur serveur',
    ...
    const createCampagne = async (data) => {
        ...
        tabError.value = '';
        succesMessage.value = '';
        axios.post('campagneperformances', data)
            .then((response) => {
                if (response.data.data.code === 500) {
                    ...
                    router.push({name: 'campagne.performance.index', params: {messages: succesMessage.value}});
                }
            })
            .catch((error) => {
                $('#btn-save').html('Enregistrer')
    ...
    const updateCampagne = async (id) => {
        ...
        tabError.value = '';
        succesMessage.value = '';
        axios.patch('campagneperformances/' + id, campagne)
            .then((response) => {
                if (response.data.data.code === 500) {
                    Swal.fire({
                        title: 'Erreur serveur',
    ...
    const deleteCampagne = async (id, idbtn) => {
        axios.delete('campagneperformances/' + id)
            .then((response) => {
