<template>
    <div>
        <div class="flex flex-col">
            <titre-application big-title="LISTE SERVICES" small-title="Services" name-page="Liste"></titre-application>
            ...
            <div>
                <div class="mb-2">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <input v-model="req" @keyup="search" type="text" class="form-control" placeholder="Rechercher un service par son libellé ou code">
                        </div>
                    </div>
                ...
                <div class="py-1">
                    ...
                    <div v-else-if="services.data.length>0" class="shadow2">
                            ...
                            <template v-for="service in services.data" :key="service.id">
                                <tr>
                                    <td class="px-5 py-1 border-b border-gray-200 bg-white text-md"
                                        v-text="service.code"></td>
                                    <td class="px-5 py-1 border-b border-gray-200 bg-white text-md"
                                        v-text="service.libelle"></td>
                                    <td v-if="service.departement===null"
                                        class="px-5 py-1 border-b border-gray-200 bg-white text-md">
                                        -
                                    </td>
                                    <td v-else class="px-5 py-1 border-b border-gray-200 bg-white text-md">
                                        {{service.departement.libelle}}
                                    </td>
                                    <td class="px-5 py-1 border-b border-gray-200 bg-white text-md"
                                        v-text="service.direction.libelle"></td>
                                    <td class="px-3 py-1 border-b border-gray-200 bg-white text-sm">
                                        <router-link :to="{ name: 'services.edit', params: { id: service.id } }"
                                                     class="btn-green text-decoration-none px-3 py-2 text-white"><i
                                            class="fa fa-edit"></i></router-link>
                                        &nbsp;&nbsp;<button @click="getIdRessource(service.id)"
                    ...
                    <div v-else class="alert alert-info">
                        <b class="text-red-600 uppercase">Aucune ressource disponible dans la base de données</b>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        name: "serviceIndex",
        components: {TitreApplication,Paginate, NavbarApp, MenuApp},
        props: {
            messages: {
                required: false,
                default: ''
        ...
        setup() {
            const req = ref('');
            const {loadRessource,services,countData,countPage,getRessources,searchRessource,paginateData, deleteService, getServices} = useServices();
