<template>
    <div>
        <div class=" flex flex-col">
            <titre-application big-title="VALIDATION PERFORMANCES GROUPE" small-title="Consultation"
            ...
            <div class="col-md-8">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <select @change="getPerformanceService" v-model="form.type" id="typeEva" class="rounded-md form-control">
...
<!--                    </div>-->
                ...
                <div v-if="salaries.length>0">
                    <table class="mt-4 min-w-full leading-normal table table-bordered table-striped"
                        ...
                        <tbody>
                        <template v-for="salarie in salaries">
                            <tr>
...
<!--                    </div>-->
                    <div class="offset-1 col-md-10">
                        <br>
                        <button :disabled="salaries.length===0 || typeEvaluation.code==='EMP'" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#staticBackdrop">VALIDER TOUTES
                            LES EVALUATIONS
                        </button>
                    </div>
                    <br><br>
            ...
            <div class="modal-dialog modal-dialog-centered relative w-auto pointer-events-none">
                <div class="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-clip-padding rounded-md outline-none text-current">
                    <div style="background-color: #acb3c4" class="modal-header flex flex-shrink-0 items-center justify-between  border-b border-gray-200 rounded-t-md">
                        <h5 class="text-red-600 font-medium leading-normal text-gray-800" id="exampleModalLabelp">
                    ...
                    <div class="modal-body">
                        <b ><h5>Cette action va valider les évaluations des collaborateurs du service et la rendre disponible pour le directeur du département. </h5></b>
        ...
        <!-- Modal auto evaluation annuelle-->
        <div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto"
                        ...
                        <div v-else>
                            ...
                            <table class="table table-striped">
                                <tr style="border: black 1px solid;background-color:  #0a6779;color: white">
                                    ...
                                    <td style="border: black 1px solid" class="hauteur"><b>Commentaire Evalué</b></td>
...
<!--                            </div>-->
                    ...
                    <div class="modal-footer flex items-center justify-end  border-t border-gray-200">
                        ...
                        <button id="btn-confirmRejet" @click="invalidEvaluation(form1.evaluation.id)" class="bg-red-600 hover:bg-red-600 text-white font-bold py-2 px-4">Rejeter</button>
    ...
    export default {
        ...
        setup() {
            ...
            const getPerformanceService = () => {
                ...
                form.start = true;
                axios.get('performance/service/'+form.type)
                    .then((response) => {
                        if (response.data.data.code===500){
                            ...
                            salaries.value = response.data.data.salaries;
                            if(salaries.value.length===0){
                                Swal.fire({
                                    title: 'ALERT',
                                    text: 'Aucune évaluations non validée disponible pour les collaborateurs',
                                    icon: 'warning',
                                });
                            }
                            form.start = false;
            ...
            const getTotalPerformance = () => {
                let value = 0;
                let totalEvaluation =  salaries.value.length;
                salaries.value.forEach(function (item) {
            ...
            const validationPerformance = async () => {
                let data = reactive({
                    ...
                    bonus: form.bonus,
                    evaluations: salaries.value,
                })

                await axios.post('validation/performance/service/' + 0 + '/' + form.type, {...data})
                    .then((response) => {
                        if (response.data.data.code === 500) {
                            Swal.fire({
                                title: 'Erreur serveur',
            ...
            const validationUnitaire = (idEval,go=false)=>{
                if (!go){
                    ...
                    sleep(100).then(()=>{
                        Swal.fire({
                            title: 'SUCCES',
                            text: 'Evaluation validée avec succès.',
                            icon: 'success',
                        });
                        form.start = true;
                        axios.get('performance/service/'+form.type)
