<template>
    <div>
        <titre-application big-title="LISTE FORMATION" small-title="Formations" name-page="Liste"></titre-application>
        <div class=" flex flex-col">
            ...
            <div>
                <div class="mb-2">
                    <div class="row">
                        <div class="mt-1 col-md-2">
                            <router-link  :to="{name:'formations.create'}" class="uppercase btn-green text-decoration-none bg-green-600 px-3 py-2 text-white">
                ...
                <div class="py-1">
                    ...
                    <div  v-else-if="formations.length>0" class="shadow  overflow-hidden">
                        <table class="min-w-full leading-normal table table-bordered table-striped" style="background-color: #acb3c4">
                            <thead>
                            <tr>
                            ...
                            <template v-for="formation in formations" :key="formation.id">
                                <tr>
                                    <td class="px-3 font-bold py-1 border-b border-gray-200 bg-white text-sm"
                                    ...
                                    <td class="px-3 py-1 border-b border-gray-200 bg-white text-md">
                                        <span v-if="formation.type_formation!==null">{{formation.type_formation.libelle}}</span>
                                        <span v-else>-</span>
                                    </td>
                                    <td class="px-3 py-1 border-b border-gray-200 bg-white text-sm">
                                        <router-link  :to="{ name: 'formations.edit', params: { id: formation.id } }"
                                                      class="btn-green text-decoration-none px-3 py-2 text-white"><i class="fa fa-edit"></i></router-link>
                                        &nbsp;&nbsp;<button @click="getIdRessource(formation.id)"  class="bg-red-600 hover:bg-red-600 text-white px-3 py-2" data-bs-toggle="modal" data-bs-target="#staticBackdrop"><i class="fa fa-trash"></i></button>
                                    </td>
                                </tr>
                            </template>
                            </tbody>
                ...
                <div class="modal-dialog modal-dialog-centered relative w-auto pointer-events-none">
                    <div class="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-clip-padding rounded-md outline-none text-current">
                        ...
                        <div class="modal-footer flex items-center justify-end  border-t border-gray-200">
                            ...
                            <button id="btn-confirm" @click="trashRessource(ressource.id)"  class="bg-red-600 hover:bg-red-600 text-white font-bold py-2 px-4">OUI</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        setup(){
            const  {loadRessource,formations,deleteFormation,getFormations} = useFormation();
            ...
            const trashRessource = (id) => {
                ...
                deleteFormation(id,'btn-fermer')
            }

            onMounted(getFormations);
