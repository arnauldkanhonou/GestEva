<template>
    <div class="flex flex-col">
        <titre-application big-title="LISTE CATEGORIES SOCIAUX PROFESSIONNELLES"
                           small-title="Catégories-profressionnelles"
            ...
            <div class="mb-2">
                <div class="row">
                    <div class="mt-1 col-md-2">
                        <router-link :to="{name:'categorie.professionnelles.create'}"
                    ...
                    <div class="col-md-6">
                        <input type="text" class="form-control"
                               placeholder="Rechercher une catégorie par code">
                    </div>
                </div>
            </div>
            <div class="py-1">
                ...
                <div v-else-if="categorieProfessionelles.length>0" class="shadow2">
                    <table class="min-w-full leading-normal table table-bordered table-striped"
                        ...
                        <tbody>
                        <template v-for="categorie in categorieProfessionelles" :key="categorie.id">
                            <tr>
                                <td class="px-5 py-1 border-b border-gray-200 bg-white text-md"
                                    v-text="categorie.code"></td>
                                <td class="px-5 py-1 border-b border-gray-200 bg-white text-md"
                                    v-text="categorie.libelle"></td>
                                <td class="px-5 py-1 border-b border-gray-200 bg-white text-md"
                                    v-text="categorie.categorie.libelle"></td>
                                <td class="px-5 py-1 border-b border-gray-200 bg-white text-md"
                                    v-text="categorie.smc"></td>
                                <td class="px-3 py-1 border-b border-gray-200 bg-white text-sm">
                                    <router-link
                                        :to="{ name: 'categorie.professionnelles.edit', params: { id: categorie.id } }"
                                        class="btn-green text-decoration-none px-3 py-2 text-white"><i
                                        class="fa fa-edit"></i></router-link>
                                    &nbsp;&nbsp;<button @click="getIdRessource(categorie.id)"
...
<script>
    ...
    export default {
        ...
        setup() {
            const {loadRessource, categorieProfessionelles, deleteCategorieProfs, getCategorieProfessionnelles} = useCategorieProfessionnelle();
            ...
            const trashRessource = (id) => {
                ...
                deleteCategorieProfs(id, 'btn-fermer')
            }

            onMounted(getCategorieProfessionnelles);
