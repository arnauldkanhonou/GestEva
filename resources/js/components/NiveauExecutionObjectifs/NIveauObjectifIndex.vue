<template>
    <div class="flex flex-col">
        <titre-application big-title="LISTE NIVEAU D'EXECUTIONS DES OBJECTIFS" small-title="Niveau-Exécutions-Objectifs"
        ...
        <div>
            <div class="mb-2">
                <div class="row">
                    <div class="mt-1 col-md-2">
                        <router-link :to="{name:'niveau.execution.objectifs.create'}"
                                     class="btn-green text-decoration-none bg-green-600 px-3 py-2 text-white">
            ...
            <div class="py-1">
                ...
                <div v-else-if="niveauObjectifs.length>0" class="shadow2">
                    <table class="min-w-full leading-normal table table-bordered table-striped"
                        ...
                        <tr>
                            ...
                            <th class="px-5 py-2 border-b-2 border-gray-200  text-left text-md font-semibold uppercase tracking-wider">
                                <b class="text-white">Appréciation</b>
                            </th>
                            <th class="px-5 py-2 border-b-2 border-gray-200  text-left text-md font-semibold uppercase tracking-wider">
                                <b class="text-white">Point</b>
                            </th>
                            <th class="px-5 py-2 border-b-2 border-gray-200  text-left text-md font-semibold uppercase tracking-wider">
                        ...
                        <template v-for="niveau in niveauObjectifs" :key="niveau.id">
                            <tr>
                                <td class="px-5 py-1 border-b border-gray-200 bg-white text-md"
                                    v-text="niveau.libelle"></td>
                                <td class="px-5 py-1 border-b border-gray-200 bg-white text-md"
                                    v-text="niveau.appreciation"></td>
                                <td class="px-5 py-1 border-b border-gray-200 bg-white text-md"
                                    v-text="niveau.point"></td>
                                <td class="px-3 py-1 border-b border-gray-200 bg-white text-sm">
                                    <router-link
                                        :to="{ name: 'niveau.execution.objectifs.edit', params: { id: niveau.id } }"
                                        class="btn-green text-decoration-none px-3 py-2 text-white"><i
                                        class="fa fa-edit"></i></router-link>
                                    &nbsp;&nbsp;<button @click="getIdRessource(niveau.id)"
...
<script>
    ...
    export default {
        ...
        setup() {
            const {loadRessource,niveauObjectifs, deleteRessource, getNiveauObjectifs} = useNiveauExecutionObjectifs();
