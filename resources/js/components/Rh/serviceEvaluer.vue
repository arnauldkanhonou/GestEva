<template>
    <div>
        <div class=" flex flex-col">
            <titre-application big-title="Services EVALUés" small-title="services"
...
<!--                            <button title="Télécharger" class="btn-sm bg-blue-600 text-white px-3 py-2 rounded">
                        ...
                        <div class="col-md-2">
                            ...
                            <select @change="loadServiceEvaluer" v-model="form.type"  class="form-select">
                        ...
                        <div class="col-md-4">
                            ...
                            <select @change="loadServiceEvaluer" v-model="form.annee" name="" class="form-select">
                ...
                <div class="py-1">
                    <div class="alert alert-success">
                        ...
                        <strong class="uppercase" v-else> Liste des services <b class="text-red-600">non évalués</b> </strong>
                    </div>
                    ...
                    <div v-else-if="form.serviceEvaluer.length>0">
                        <table class="mt-4 min-w-full leading-normal table table-bordered table-striped"
                            ...
                            <tr>
                                ...
                                <th class="py-2 px-6 border-b-2 border-gray-200  text-left text-md font-semibold uppercase tracking-wider">
                                    <b class="text-white">Nombre total</b>
                                </th>
                                <th class="py-2 px-6 border-b-2 border-gray-200  text-left text-md font-semibold uppercase tracking-wider">
                                    <b class="text-white">Nombre Evalué. </b>
                                </th>
                                <th class="py-2 px-6 border-b-2 border-gray-200  text-left text-md font-semibold uppercase tracking-wider">
                                    <b class="text-white">Reste à evaluer</b>
                                </th>

                            </tr>
                            </thead>
                            <tbody>
                            <template v-for="service in form.serviceEvaluer">
                                <tr>
                                    ...
                                    <td class="py-2 border-b border-gray-200 bg-white text-md"
                                        v-text="service.nbreTotal">
                                    </td>
                                    <td class="py-2 border-b border-gray-200 bg-white text-md"
                                        v-text="service.nbreEvaluer"></td>
                                    <td class="py-2 border-b border-gray-200 bg-white text-md">
                                        {{service.nbreTotal-service.nbreEvaluer}}
                                    </td>
                                </tr>
                            </template>
                            </tbody>
...
<script>
    ...
    export default {
        ...
        setup(props){
            ...
            const loadServiceEvaluer = () => {
                form.serviceEvaluer = [];
                axios.get('liste/service/' + form.annee+'/'+form.type)
                    .then((response) => {
                        if (response.data.data.code === 500){
                            ...
                            if (response.data.data.listes.length>0){
                                form.serviceEvaluer = response.data.data.listes;
                            }
                        }
