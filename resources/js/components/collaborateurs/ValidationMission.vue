<template>
    <div>
        <div class=" flex flex-col">
            <titre-application big-title="MISSIONS COLLABORATEURS" small-title="Missions" :name-page="intitule"></titre-application>
                ...
                <div class="mb-2">
                    <div class="row">
                        <div class="mt-1 col-md-4">
                            ...
                            &nbsp;&nbsp;
                            <router-link :to="{name:'missions.create',params: {idCol: id,intituleCol: intitule} }"
                                         class="uppercase btn-green text-decoration-none bg-green-600 px-3 py-2 text-white">
                            ...
                            <template v-for="mission in missions" :key="mission.id">
                                <tr>
                                    <td class="px-3 py-2 border-b border-gray-200 bg-white text-md">
                                        <input v-if="!mission.valider" :id="'mission'+mission.id"
                                    ...
                                    <td class="px-3 py-2 border-b border-gray-200 bg-white text-md"
                                        v-text="mission.annee.libelle"></td>
                                    <td class="px-3 py-2 border-b border-gray-200 bg-white text-sm">
                                        <router-link v-if="!mission.valider" title="Modifier la maission"
                                                     :to="{ name: 'missions.edit', params: { id: mission.id,idCol:id,intituleCol: intitule } }"
                                                     class="btn-green text-decoration-none px-3 py-2 text-white"><i
                                                class="fa fa-edit"></i></router-link>
                                        <button v-else disabled type="button" class="text-decoration-none cursor-auto btn-green px-3 py-2 text-white"><i  class="fa fa-edit"></i></button>
...
<script>
    ...
    export default {
        ...
        setup(props) {
            ...
            const {mission,succesMessage,deleteMission, validateMission} = useMission();
            ...
            const trashRessource = (id) => {
                ...
                sleep(100).then(() => {
                    getMissionCollaborateur(props.id)
                })
            }
            ...
            const validateRessource = (id, idbtnConfirm, idbtn) => {
                ...
                validateMission(data, idbtnConfirm, idbtn,idCollaborateur,intitule);
            }
