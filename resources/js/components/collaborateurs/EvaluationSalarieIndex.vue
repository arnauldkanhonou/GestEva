<template>
    <div>
        <div class=" flex flex-col">
            <titre-application big-title="MES EVALUATIONS" small-title="Evaluations"
            ...
            <div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 ">
                            ...
                            <select @change="filtreEvaluation()" id="filtre" class="form-select">
                                <option value="EA">Evaluation annuelle</option>
                                <option value="EMP">Evaluation mi-parcours</option>
                            </select>
                        </div>
                        <div class="col-md-4">
                            <label for=""><strong>Filtrer Par Années</strong></label>
                            <select @change="filtreByAnnee" v-model="annee" name="" id="" class="form-select">
                ...
                <div class="py-1">
                    ...
                    <div v-else-if="evaluationsCollaborateur.data.length>0" class="shadow2">
                ...
                <div class="modal-dialog modal-lg modal-dialog-centered relative w-auto pointer-events-none">
                    <div
                            ...
                            <div v-else>
                                ...
                                <table class="table table-striped">
                                    <tr style="border: black 1px solid;background-color:  #0a6779;color: white">
                                        ...
                                        <td style="border: black 1px solid"><b>Commentaire Evalué</b></td>
                                        <td style="border: black 1px solid"><b>Commentaire Evaluateur</b></td>
                                    </tr>
                                    ...
                                    <tr v-for="objectif in form1.objectifs" style="border: black 1px solid;">
                                        ...
                                        <td style="border: black 1px solid" align="right"><b>{{objectif.commentaireEvaluer}}</b></td>
                                        <td style="border: black 1px solid" align="right"><b>{{objectif.commentaireResp}}</b></td>
                                    </tr>
                                ...
                                <table class="table table-striped">
                                    ...
                                    <tr style="border: black 1px solid">
                                        ...
                                        <td style="border: black 1px solid" class="hauteur" align="right"><b
                                            class="text-red-600">{{form1.total}}</b></td>
                                    </tr>
                                </table>
                                <hr>
                        ...
                        <div class="modal-footer flex items-center justify-end  border-t border-gray-200">
                            ...
                            <button v-if="!form1.evaluation.transmis" @click="displayFormAutoEvaluation" id="btn-edit"
                                    class="bg-green-600 hover:bg-green-600 text-white font-bold py-2 px-4"
                            ><i class="fa fa-edit"></i>Modifier
                            </button>

<!--                            <button v-if="!form1.evaluation.transmis" id="btn-confirm-send"-->
<!--                                    @click="sendEvaluation(form1.evaluation.id)"-->
<!--                                    class="btn-green text-white font-bold py-2 px-4"><i class="fa fa-send"></i>TRANSMETTRE-->
<!--                            </button>-->
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-dialog modal-lg modal-dialog-centered relative w-auto pointer-events-none">
                    <div
                        ...
                        <div class="modal-footer flex items-center justify-end  border-t border-gray-200">
                            ...
                            <button v-if="!form.evaluation.transmis" @click="displayFormAutoEvaluation(false)" id="btn-edit-miparcours"
                                    class="bg-red-600 hover:bg-red-600 text-white font-bold py-2 px-4"
                            ><i class="fa fa-edit"></i>Modifier
                            </button>

                            <button v-if="!form.evaluation.transmis" id="btn-confirm-send-mi-parcours"
                                    @click="sendEvaluation(form.evaluation.id)"
                                    class="btn-green text-white font-bold py-2 px-4"><i class="fa fa-send"></i>TRANSMETTRE
                            </button>
...
<script>
    ...
    export default {
        ...
        setup(props) {
            ...
            const {loadRessource, annees, evaluationsCollaborateur,
                countPage, filtreEvalautionSalarie,filtreEvaluationByType,
                getAnnees, getEvaluationSalarie,paginateData
            } = useCollaborateur();
            ...
            onMounted(() => getEvaluationSalarie())
            ...
            const calculPointObjectif = (objectifs) => {
                objectifs.forEach((objectif,index) => {
                    let note = parseInt(objectif.noteObtenue);
                    pointObjec.value = parseInt(pointObjec.value) + note;
                })
                console.log(pointObjec.value);
                return pointObjec.value
            }

            const calculPointPerfor = (note) => {
                pointPerfor.value = form1.totalperformance + note;
                return pointPerfor
            }

            const displayFormAutoEvaluation = (autoEvaluation =true) => {
                if (autoEvaluation){
                    $('#btn-fermer')[0].click();
                    router.push({name: 'auto.evaluations.index'});
                }else {
                    $('#btn-fermer-mipacours')[0].click();
                    router.push({name: 'evaluation.mis.pacours'});
                }

            }

            const sendEvaluation = (idEvaluation) => {
                $('#btn-confirm-send').html('<i class="fas fa-circle-notch fa-spin fa-2x"></i>')
                axios.get('send/evaluation/for/chief/' + idEvaluation)
                    .then((response) => {
                        if (response.data.data.code === 500) {
                            ...
                            $('#btn-confirm-send').html('Transmettre')
                            $('#btn-fermer')[0].click();
                            $('#btn-fermer-mipacours')[0].click();
                            Swal.fire({
                                title: 'Succès',
            ...
            const rapportEvaluation = (id, display = false) => {
                ...
                axios.get('check/mission/collaborateur/valider/' + user.id + '/' + id)
                    .then((response) => {
                        if (response.data.data.code === 500) {
                            Swal.fire({
                                title: 'Erreur',
                            ...
                            console.log('ok');
                            load.value = true;
                            window.location.href = baseUrl + 'api/rapport/evaluation/collaborateur/' + id + '/' + display;
                        }
                    })
                    .catch((error) => {
            ...
            const filtreEvaluation = () => {
                ...
                filtreEvaluationByType(user.id,filtre,false);
            }

            const filtreByAnnee = () => {
                filtreEvalautionSalarie(annee.value)
            }
