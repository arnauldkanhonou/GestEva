<template>
    <div>
        <titre-application big-title="EVALUATION MI-PARCOURS DE" small-title="evaluation"  :name-page="intitule"></titre-application>
        <div v-if="load" class="mt-4">
                        ...
                        <tr>
                            ...
                            <td style="width: 40%" class="py-2 border-b border-gray-200 bg-white text-md">
                                <input class="w-4 h-4" type="radio" :name="'niveau'+parseInt(objectif.id)"
                                       :checked="objectif.NivRealisationMPEvaluer==='largement dépassé'"
                                ...
                                <input class="w-4 h-4" type="radio" :name="'niveau'+parseInt(objectif.id)"
                                       :checked="objectif.NivRealisationMPEvaluer==='atteint'"
                                       @click="setNiveauRealisationObjectif('atteint',objectif.id)"
                                       value="atteint"><b>B(<b class="text-red-600">atteint</b>)</b>&nbsp;&nbsp;&nbsp;
                                <input class="w-4 h-4" type="radio" :name="'niveau'+parseInt(objectif.id)"
                                       :checked="objectif.NivRealisationMPEvaluer==='partiellement atteint'"
                                ...
                                atteint</b>)</b>&nbsp;&nbsp;&nbsp;
                                <input class="w-4 h-4" type="radio" :name="'niveau'+parseInt(objectif.id)"
                                       :checked="objectif.NivRealisationMPEvaluer==='non atteint'"
                            ...
                            <td style="width: 5%" class=" py-1 border-b border-gray-200 bg-white text-sm">
                                &nbsp;&nbsp;<button @click="setCommentaireObjectif(objectif.id,objectif.commentaireEvaluerMP,false)"
            ...
            <div class="mt-5">
                <button data-bs-toggle="modal" data-bs-target="#staticBackdrop1"
                    ...
                    class="fa fa-book"></i> AJOUTER AUTRES REALISATIONS
                </button>
            </div>
            <table class="mt-4 min-w-full leading-normal table table-bordered table-striped"
                   style="background-color:#acb3c4 ">
                ...
                <div class="row mt-4">
                    <div class="col-md-12">
                        <label for="synthese" class="mb-1 block text-base font-medium text-[#07074D]">
                            <b> Commentaire du salarié<i class="text-red-600">*</i></b>
                        </label>
            ...
            <div class="mt-5 row">
                <div class="offset-6 col-md-3">
                    <button data-bs-toggle="modal" data-bs-target="#staticBackdrop2"
                        ...
                        <i class="fa fa-check-circle"></i> <b>ENREGISTRER ET ENVOYER AU N+1</b>
                    </button>
                </div>

                <div class="col-md-3">
                    <button id="saveEvaluation"  @click="clotureEvaluation(false)"
                            class="col-md-12 bg-primary text-white px-4 py-2" type="button"><b>ENREGISTRER</b>
                    </button>
                </div>
            </div>
                        ...
                        <div class="container">
                            <div class="col-md-12">
                                <label for="realisation" class="mb-1 block text-base font-medium text-[#07074D]">
                                    <b>Réalisation<i class="text-red-600">*</i></b>
                                </label>
                            ...
                            <div class="mt-4 col-md-12">
                                <label for="difficulte" class="mb-1 block text-base font-medium text-[#07074D]">
                                    <b>Difficultés<i class="text-red-600">*</i></b>
                                </label>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div
                        ...
                        <div class="container">
                            <div>
                                <strong class="text-red-600">
                                    Cette action va valider votre évaluation et vous ne serez plus en
                                    mesure d'apporter des modifications sur cette dernière.
                                    <br>Voulez-vous vraiment enregistrer et envoyer l'évaluation ?
                                </strong>
                            </div>
                        </div>
                    ...
                    <div class="modal-footer flex items-center justify-end  border-t border-gray-200">
                        ...
                        <button id="btn-valider" @click="clotureEvaluation(true)"
...
export default {
    ...
    setup(props, context) {
        ...
        onMounted(() => {
            axios.get('init/evaluation/miparcours/' + props.id+'/evaluationMiparcoursSalarieByResp')
                .then((response) => {
                    if (response.data.data.code === 500) {
                        ...
                        form.commentaireEvaluer = resp.evaluation.commentaireEvaluer
                        evaluation.value = resp.evaluation;
                        load.value = true;
                        code_retour.value=500;
        ...
        onMounted(() => getObjectifCollaborateurValider(props.id,'evaluationMiparcoursSalarieByResp'));
        ...
        const setCommentaireObjectif = (idObjectif,commentaire, go = false) => {
            ...
            axios.post('add/commentaire/miparcours/' + form.idObjectif, data)
                .then((response) => {
                    getObjectifCollaborateurValider(props.id,'evaluationMiparcoursSalarieByResp')
                });
        }
        ...
        const addRealisationAmiParcours = () => {
            ...
            if (form.realisation.libelle === '' || form.realisation.difficulte === '') {
                alert('Veuillez saisir toutes les informations de realisation');
                return;
            }
            let data = {
        ...
        const clotureEvaluation = (send=false)=>{
            let data = {
                type:'evaluer',
                send: send
            }
            axios.post('cloture/evaluation/miparcours/' + evaluation.value.id,{...data})
                .then((response) => {
                    if (response.data.data.code===500){
                        ...
                        let succesMessage = 'Votre évaluation est bien enregistrés et envoyée à votre N+1.'

                        if (!send){
                            succesMessage = 'Votre évaluation est bien enregistrée. Vous devez la transmettre à votre responsable pour validation.'
                        }
