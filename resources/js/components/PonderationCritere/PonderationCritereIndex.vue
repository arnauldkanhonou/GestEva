<template>
    <div class="flex flex-col">
        <titre-application big-title="LISTE PONDERATIONS CRITERES" small-title="Pondérations-Critères"
        ...
        <div>
            <div class="mb-2">
                <div class="row">
                    <div class="mt-1 col-md-2">
                        <router-link :to="{name:'ponderation.criteres.create'}"
                    ...
                    <div class="col-md-6">
                        <input type="text" class="form-control"
                               placeholder="Rechercher une pondération">
                    </div>
                </div>
            ...
            <div class="py-1">
                ...
                <div v-else-if="ponderationCriteres.length>0" class="shadow2">
                    <table class="min-w-full leading-normal table table-bordered table-striped"
                        ...
                        <tbody>
                        <template v-for="ponderation in ponderationCriteres" :key="ponderation.id">
                            <tr>
                                <td class="px-5 py-1 border-b border-gray-200 bg-white text-md"
                                    v-text="ponderation.libelle"></td>
                                <td class="px-5 py-1 border-b border-gray-200 bg-white text-md"
                                    v-text="ponderation.point"></td>
                                <td class="px-3 py-1 border-b border-gray-200 bg-white text-sm">
                                    <router-link
                                        :to="{ name: 'ponderation.criteres.edit', params: { id: ponderation.id } }"
                                        class="btn-green text-decoration-none px-3 py-2 text-white"><i
                                        class="fa fa-edit"></i></router-link>
                                    &nbsp;&nbsp;<button @click="getIdRessource(ponderation.id)"
...
<script>
    ...
    export default {
        ...
        setup() {
            const {loadRessource,ponderationCriteres, deleteRessource, getPonderations} = usePonderationCritere();
